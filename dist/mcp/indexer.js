 ...
 */
function chunkText(text, filePath, layer, project) {
    ...
    const lineStarts = [0]; // byte offset where each line begins
    ...
    function charOffsetToLine(offset) {
 ...
 */
export async function indexFile(db, filePath, layer, project, provider) {
    let content;
    try {
        content = await fsAsync.readFile(filePath, 'utf-8');
    }
    catch (err) {
        console.warn(`[indexer] skipping unreadable file: ${filePath}`, err.message);
        return 0;
    }
    if (!content.trim())
        return 0;
  ...
  `);
    const runTransaction = db.transaction((chunks) => {
 ...
 */
export async function indexDirectory(db, dirPath, layer, project, provider) {
 ...
 */
export async function backfillEmbeddings(db, provider) {
    const paths = db.prepare("SELECT DISTINCT path FROM chunks WHERE embedding IS NULL").all();
    for (const { path: filePath } of paths) {
        // Detect layer and project from path
        const layer = filePath.includes('/.claude-memory/') ? 'global' : 'project';
