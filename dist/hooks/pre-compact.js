 ...
 */
function extractRecentTranscript(messages) {
...
async function main() {
        ...
        try {
            ...
            const resp = await openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: 'system',
                        content: 'You are a context-preservation assistant. Extract the most critical context from this session that would be lost after compaction. Focus on: current task state, key decisions made, open problems, important file paths. Output concise bullet points in Markdown.',
                    },
                    {
                        role: 'user',
                        content: transcript,
            ...
            console.error('[pre-compact] OpenAI call failed, falling back to plain excerpt:', err.message);
            contextNotes = '';
        }
    }
    else {
    ...
    try {
        ...
        fs.writeFileSync(COMPACT_PENDING_PATH, JSON.stringify({ timestamp: Date.now(), session_id: sessionId }));
    }
    catch {
    ...
    try {
        ...
        await indexFile(db, dailyLogPath, isGlobal ? 'global' : 'project', isGlobal ? undefined : cwd);
        db.close();
    }
    catch (err) {
        console.error('[pre-compact] re-index failed:', err.message);
    }
}
main().catch((err) => {
