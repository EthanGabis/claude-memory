...
function wasMemoryRecentlyWritten() {
    try {
 ...
 */
function findClaudeMemoryDir(dir) {
 ...
 */
async function summariseWithAI(transcriptText) {
...
Keep each field concise. If a field has no relevant info, use "N/A".`,
        ...
        console.error('[stop-hook] AI summarisation failed:', err.message);
        return null;
    }
}
/**
 ...
 */
function buildFallbackSummary(transcriptText) {
...
async function main() {
            ...
            if (!nudgeState.sessions[sessionId]) {
                ...
                if (!wasMemoryRecentlyWritten()) {
                    ...
                    process.stdout.write(JSON.stringify({
                        decision: 'block',
                        reason: "Before ending: you had a substantive session. Save any durable facts (preferences, decisions, debugging insights) to memory via memory_save(target='memory'). If nothing to save, say 'nothing to save' and I'll let you end.",
                    }) + '\n');
                    process.exit(0);
                }
            }
    ...
    try {
        ...
        console.error('[stop-hook] Re-indexing failed:', err.message);
    }
}
main().catch((err) => {
