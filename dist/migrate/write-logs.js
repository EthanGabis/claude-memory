...
function renderSession(session) {
    ...
    if (session.observations.length > 0) {
        ...
        for (const obs of session.observations) {
            ...
            if (obs.filesModified)
                lines.push(`Files: ${obs.filesModified}`);
        }
    }
    return lines.join("\n");
}
function renderDayFile(date, sessions) {
...
// ---------------------------------------------------------------------------
function safeWriteLogFile(filePath, content) {
...
// ---------------------------------------------------------------------------
function processProject(projectFile) {
...
// ---------------------------------------------------------------------------
function processOrphans(sessions) {
    const dateGroups = groupByDate(sessions);
    let filesWritten = 0;
    for (const [date, daySessions] of dateGroups) {
        const content = renderDayFile(date, daySessions);
...
function main() {
    ...
    for (const entry of manifest.projects) {
        ...
        try {
            ...
            console.warn(`[write-logs] Skipping corrupt/unreadable project file: ${projectJsonPath}`, err.message);
            continue;
        }
    ...
    if (manifest.orphanCount > 0 && manifest.orphansFile) {
        ...
        if (!existsSync(orphansPath)) {
            ...
            try {
                ...
                console.warn(`[write-logs] Skipping corrupt/unreadable orphans file: ${orphansPath}`, err.message);
            }
            if (orphanFile) {
