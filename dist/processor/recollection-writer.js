...
// ---------------------------------------------------------------------------
export async function writeRecollections(sessionId, userMessage, userMessageUuid, projectName, previousEmbedding, embedProvider, db) {
    ...
    if (ftsQuery.length > 0) {
        try {
            bm25Rows = db.prepare('SELECT rowid, bm25(episodes_fts) as score FROM episodes_fts WHERE episodes_fts MATCH ? LIMIT 50').all(ftsQuery);
        }
        catch {
    ...
    const episodes = db.prepare(`SELECT rowid, id, summary, importance, embedding, created_at, accessed_at, access_count
     FROM episodes
    ...
    for (const episode of episodes) {
        // Vector similarity
        const vectorSim = cosineSimilarity(queryBlob, episode.embedding);
    ...
    const updateStmt = db.prepare('UPDATE episodes SET accessed_at = ? WHERE id = ?');
    for (const s of top3) {
...
// Atomic file write
// ---------------------------------------------------------------------------
function writeRecollectionFile(sessionId, messageUuid, bites) {
    ...
    const tmpPath = path.join(RECOLLECTIONS_DIR, `${sessionId}.tmp`);
