...
async function graduateEpisodes(db) {
    ...
    LIMIT ?
  `).all(GRADUATION_MIN_ACCESS, MAX_GRADUATED_PER_CYCLE * 2);
    ...
    return withFileLock(GLOBAL_MEMORY_PATH + '.lock', async () => {
        ...
        for (const candidate of candidates) {
            if (graduated >= MAX_GRADUATED_PER_CYCLE)
                break;
            ...
            if (newContent.includes(entry.trim()))
                continue;
            newContent += entry;
            graduated++;
        }
...
async function compressStaleEpisodes(db) {
  ...
  `).all(cutoff, COMPRESS_MAX_ACCESS);
    if (stale.length === 0)
        return 0;
    const compressStmt = db.prepare('UPDATE episodes SET full_content = NULL WHERE id = ?');
    let compressed = 0;
