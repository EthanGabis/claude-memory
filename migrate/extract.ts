...
function main() {
  ...
  if (!existsSync(dbPath)) {
    ...
    console.error(
      "Make sure claude-mem is installed and has been run at least once."
    );
    process.exit(1);
  }
  ...
  const availableTables: Record<string, boolean> = {};
  ...
  if (!availableTables.sdk_sessions) {
    console.error(
      "ERROR: sdk_sessions table is required but missing. Cannot proceed."
    );
    db.close();
    process.exit(1);
  }
  ...
  // --- Query summaries keyed by memory_session_id ---
  type SummaryRow = {
  ...
  const summariesMap = new Map<string, SummaryRow[]>();
  ...
  if (availableTables.session_summaries) {
    ...
    for (const row of rows) {
      ...
      if (!summariesMap.has(key)) summariesMap.set(key, []);
      summariesMap.get(key)!.push(row);
    }

    console.log(
      `Found ${rows.length} summary rows across ${summariesMap.size} sessions.`
    );
  }

  // --- Query observations keyed by memory_session_id ---
  type ObservationRow = {
    memory_session_id: string;
    title: string | null;
    narrative: string | null;
  ...
  if (availableTables.observations) {
    ...
    for (const row of rows) {
      const key = row.memory_session_id;
      if (!observationsMap.has(key)) observationsMap.set(key, []);
      observationsMap.get(key)!.push(row);
    }

    console.log(
      `Found ${rows.length} observation rows across ${observationsMap.size} sessions.`
    );
  }
  ...
  const orphanSessions: SessionEntry[] = [];
  ...
  const manifestProjects: ManifestEntry[] = [];
  ...
  for (const [slug, projectFile] of projectFiles) {
    ...
    console.log(
      `Wrote ${projectFile.sessions.length} sessions → ${outPath}`
    );
  ...
  if (orphanSessions.length > 0) {
    ...
    console.log(
      `Wrote ${orphanSessions.length} orphan sessions → ${orphansPath}`
    );
  }

  // Manifest
  const manifest: Manifest = {
    projects: manifestProjects.sort(
